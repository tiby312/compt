...
impl<T> CompleteTree<T>{
    ...
    #[inline]
    pub fn from_dfs_preorder<F:FnMut()->T>(mut func:F,height:usize)->CompleteTree<T>{
        assert!(height>0,"Height must be atleast 1");
    ...
    #[inline]
    pub fn dfs_preorder_iter(&self)->std::slice::Iter<T>{
        self.nodes.iter()
    }

    #[inline]
    pub fn dfs_preorder_iter_mut(&mut self)->std::slice::IterMut<T>{
        self.nodes.iter_mut()
    }
    #[inline]
...
impl<'a,T:'a> Visitor for Vistr<'a,T>{
    ...
    fn next(self)->(Self::Item,Option<((),Self,Self)>){
        ...
        if remaining.len()==1{
            ...
            let (node,rest)=remaining.split_at(1);
            let div=rest.len()/2;
            let (left,right)=rest.split_at(div);
            (&node[0],Some(((),Vistr{remaining:left},Vistr{remaining:right})))
        }
    }
...
impl<'a,T:'a> Visitor for VistrMut<'a,T>{
    ...
    fn next(self)->(Self::Item,Option<((),Self,Self)>){
        ...
        if remaining.len()==1{
            ...
            let (node,rest)=remaining.split_at_mut(1);
            let div=rest.len()/2;
            let (left,right)=rest.split_at_mut(div);
            (&mut node[0],Some(((),VistrMut{remaining:left},VistrMut{remaining:right})))
        }
    }
