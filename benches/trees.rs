...
fn bench_bfs_dfs(bench:&mut Bencher){
	...
	let mut k=compt::bfs_order::CompleteTree::from_vec(vec![0;16383],14).unwrap();
	bench.iter(||{
...
fn bench_dfs_dfs(bench:&mut Bencher){
	...
	let mut k=compt::dfs_order::CompleteTree::from_vec(vec![0;16383],14).unwrap();
	bench.iter(||{
		for (a,_) in k.vistr_mut().dfs_preorder_iter(){
			*a+=1;
		}
...
fn bench_bfs_bfs(bench:&mut Bencher){
	...
	let mut k=compt::bfs_order::CompleteTree::from_vec(vec![0;16383],14).unwrap();
	bench.iter(||{
		for (a,_) in k.vistr_mut().bfs_iter(){
			*a+=1;
		}
	});
